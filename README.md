...
status      ok
outputs
  account                      13566 (SAVINGS)
  balance_before               $4820.55
  transfer_amount              $25.00
  balance_after                $4845.55
  available_after              $4845.55
  checking_13344_balance_after $1225.00
steps run   12
duration    2138 ms
model calls 0
...
|---|---|---|
| `surface.py` | 425 | **The seam.** `Surface` protocol (`observe`/`act`/`screenshot`/`wait`) + `WebSurface`, which reads the CDP accessibility tree — including inside framesets — rather than the DOM. Replay depends on the protocol, not on Playwright. |
...
| `replay.py` | 897 | The production path. Resolution, checkpoints, detectors, recovery, extraction, escalation. Imports no model client. |
