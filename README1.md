...
```bash
git clone interface_ai && cd interface
...
### 1. Reset to known balances

```bash
curl -X POST http://127.0.0.1:8099/_reset
```

Savings 13566 = `$4820.55`, checking 13344 = `$1250.00`. `/_reset` sits
**outside the agent's allowlist** — the automation can never reset the world it
is operating on.
...
| 3 | `ordinal` | 0.65 | — |
| 4 | `coords` | 0.45 | policy-gated last resort |
